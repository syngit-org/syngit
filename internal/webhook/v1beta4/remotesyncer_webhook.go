...
func SetupRemoteSyncerWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr, &syngitv1beta4.RemoteSyncer{}).
		WithValidator(&RemoteSyncerCustomValidator{}).
		Complete()
}

// +kubebuilder:webhook:path=/validate-syngit-io-v1beta4-remotesyncer,mutating=false,failurePolicy=fail,sideEffects=None,groups=syngit.io,resources=remotesyncers,verbs=create;update,versions=v1beta4,name=vremotesyncer-v1beta4.kb.io,admissionReviewVersions=v1
...
func validateRemoteSyncerSpec(r *syngitv1beta4.RemoteSyncerSpec) field.ErrorList {
	var errors field.ErrorList

	// Validate DefaultRemoteUserRef based on DefaultUnauthorizedUserMode
	if r.DefaultUnauthorizedUserMode == syngitv1beta4.Block && r.DefaultRemoteUserRef != nil {
	...
	} else if r.DefaultUnauthorizedUserMode == syngitv1beta4.UseDefaultUser && r.DefaultRemoteUserRef == nil {
	...
	if r.DefaultBlockAppliedMessage != "" && r.Strategy != syngitv1beta4.CommitOnly {
		errors = append(errors, field.Forbidden(field.NewPath("spec").Child("defaultBlockAppliedMessage"), fmt.Sprintf("should not be set if strategy is not set to \"%s\"", syngitv1beta4.CommitOnly)))
	}

	// Validate that Strategy is either CommitApply or CommitOnly
	if r.Strategy != syngitv1beta4.CommitOnly && r.Strategy != syngitv1beta4.CommitApply {
		errors = append(errors, field.Invalid(field.NewPath("spec").Child("strategy"), r.Strategy, fmt.Sprintf("must be set to \"%s\" or \"%s\"", syngitv1beta4.CommitApply, syngitv1beta4.CommitOnly)))
	}
	...
	if r.DefaultUnauthorizedUserMode != syngitv1beta4.Block && r.DefaultBranch == "" {
...
func validateRemoteSyncer(remoteSyncer *syngitv1beta4.RemoteSyncer) error {
	...
	if !slices.Contains([]syngitv1beta4.RemoteTargetUserSpecificValues{"", syngitv1beta4.RtAnnotationValueOneUserOneBranch, syngitv1beta4.RtAnnotationValueOneUserOneBranch}, syngitv1beta4.RemoteTargetUserSpecificValues(rtAnnotationUserSpecific)) {
		allErrs = append(allErrs, field.Invalid(field.NewPath("metadata").Child("annotations").Child(syngitv1beta4.RtAnnotationKeyUserSpecific), rtAnnotationUserSpecific,
			fmt.Sprintf("must be either %s or %s; got %s", string(syngitv1beta4.RtAnnotationValueOneUserOneBranch), string(syngitv1beta4.RtAnnotationValueOneUserOneBranch), rtAnnotationUserSpecific)))
	}
...
func (v *RemoteSyncerCustomValidator) ValidateCreate(ctx context.Context, remotesyncer *syngitv1beta4.RemoteSyncer) (admission.Warnings, error) {
...
func (v *RemoteSyncerCustomValidator) ValidateUpdate(ctx context.Context, oldObj, newRemotesyncer *syngitv1beta4.RemoteSyncer) (admission.Warnings, error) {
...
func (v *RemoteSyncerCustomValidator) ValidateDelete(ctx context.Context, remotesyncer *syngitv1beta4.RemoteSyncer) (admission.Warnings, error) {
